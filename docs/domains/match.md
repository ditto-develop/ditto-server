...
- **성사되지 못한 그룹은 상태를 바꾸지 않고 알림만 낸다**(`UnformedGroupNotifier`). 마감(그 주 금요일 00:00 — 채팅 개방과 같은 순간)까지 3명에 못 닿으면 수락자에게 `GROUP_NOT_FORMED`를 보낸다. 취소 플래그를 따로 두지 않는 이유: `is_active=false`가 이미 "성사되지 않음"이고, 재발송은 알림 유형의 `ONCE_PER_TARGET`(대상 = `group_match.id`)이 막아 상태 없이도 멱등이다. 다만 그 멱등은 알림 행이 살아 있는 동안만이라(30일 뒤 purge) 스캔을 최근 2주 주차로 자른다. 알림이 없던 동안 수락자는 채팅방이 왜 안 열리는지 알 수 없었다(QA BUG-071).

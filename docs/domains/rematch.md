...
- **취소 사유는 `cancel_reason`으로 저장한다**(`NOT_MUTUAL`·`MEMBER_LEFT`). 사유가 "상호 선택이 아님" 하나뿐일 때는 `status`에서 도출돼 컬럼을 두지 않았으나(ADR 0012), 탈퇴가 두 번째 사유가 되면서 값으로 구분해야 한다 — 두 선택 값만으로는 탈퇴를 알 수 없다. 중복은 성사를 취소하지 않으므로([ADR 0013](../adr/0013-rematch-duplicate-at-room-creation.md)) 사유가 되지 않는다.
...
- **탈퇴는 미성사(`WAITING`) 쌍을 취소한다**(`LeftMemberRematchCanceller`, 탈퇴 트랜잭션 안에서).

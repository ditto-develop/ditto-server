...
- 제출 응답의 재매칭 결과(`rematch`)는 **이번 제출로 상호 성사된 경우에만** 채운다. 상대의 선택 자체는 목록 조회의 `targets[].counterpartWantsRematch`로 본다(신청/수락 모델, [ADR 0031](../adr/0031-rematch-request-visible-to-counterpart.md)). 제출 커밋 뒤 컨트롤러가 `RematchNotifier`로 신청·거절 알림을 남긴다.
...
- **받은 평가 집계의 공개 기준은 3건**이며(`MemberRatingService.PUBLIC_THRESHOLD`), 미달이면 총 건수만 내려간다. 내 프로필(`/users/me/ratings`)과 타인 프로필(`/users/{id}/ratings`)이 같은 집계를 쓰고, `PublicProfileResponse.rating`도 같은 기준으로 채운다. 타인 조회의 열람 권한은 공개 프로필과 같고(차단 아님), **관계에 따라 공개 범위가 둘로 갈린다** — 매칭 성사·같은 그룹 채팅 참여는 코멘트·노쇼까지 전체를, 이번 주 매칭 후보(성사 전)는 평균·건수까지만 본다(`noShowCount=null`, `ratings=[]`). 배경·감수한 프라이버시 비용: [ADR 0020](../adr/0020-peer-profile-answer-match-summary.md), [ADR 0029](../adr/0029-peer-profile-candidate-summary-tier.md).
...
- "미완료"는 `COMPLETED가 아님`이며, 이 판정은 조회 쿼리(`findPendingByAuthorOldestFirst`·리마인드용 `findPendingAvailableBetween`)에만 둔다 — 서비스나 호출부에서 상태를 나열하지 않는다.
- **평가에는 마감이 없다.** 월요일의 평가 리마인드(`REVIEW_REMINDER`, [notification](notification.md))는 최근 7일 안에 열린 미완료 평가에 한 번 독촉할 뿐이고 제출은 계속 열려 있다.
...
- **평가를 여는 방 유형은 `MemberReview.REVIEWABLE_MATCH_TYPES` 한 곳에서 정한다**(`PERSONAL`·`GROUP`). **재매칭 채팅은 평가를 열지 않는다** — 이미 서로를 원해서 만난 자리라 다시 평가를 받지 않는다. 조립하는 쪽과 누락 복구 조회가 **같은 값을 봐야 한다**: 갈라지면 평가가 열리지 않는 방이 영원히 "평가 없음"으로 남아 복구 배치 앞자리를 점유하고, 그 뒤에 끝난 방이 복구되지 못한다(아래 한계와 같은 메커니즘인데 이쪽은 정상 동작으로 매주 쌓인다).
  - 부수 효과: 재매칭 상대의 노쇼·별점이 집계에 쌓이지 않는다. `T1`(신뢰도) 노쇼 감점과 `P1` 공개 평균 별점이 재매칭 만남을 보지 못하므로 그 트랙 착수 시 다시 판단한다.

...
- `ChatVoteCloseReason` — 마감 사유(`MEMBER`/`ROOM_ENDED`). `closed_by`의 null을 해석하게 두지 않는다. 사유·마감자는 상세 응답에도 그대로 나간다(`closedReason`·`closedBy`) — 결과 카드 문구가 진입 경로에 따라 갈리지 않게.
...
- **선택지는 생성 시 각 2~10개, 진행 중에는 멤버 누구나 하나씩 추가할 수 있다**(피그마 4.2.3 "항목 추가" 화면). 추가는 유형별 최대 10개까지고(8204) 삭제는 없다. 중복은 PLACE=`(vote, type, label)`, TIME=`(vote, meet_at)` 유일키가 최종으로 막고, 요청 내 중복은 저장 전에 8205로 거른다(UK까지 가면 INTERNAL_ERROR로 샌다). TIME은 분 단위로 절삭해 판정한다.
...
- cast·close·선택지 추가: 투표 행 잠금이 첫 접근. 방 행은 잠그지 않는다(수정하지 않고, 상태 판정은 잠금 획득 뒤에 읽어 최신 커밋을 본다).

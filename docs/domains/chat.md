...
- **나간 방은 그 사람에게 보이지 않는다.** 목록에서 빠지고(`getMyRooms`), 메시지·투표 조회도 `validateMember`가 이탈자를 거부한다. 예전에는 읽기 전용으로 남겼는데, 메시지 커서 조회에 `left_at` 컷오프가 없어 **나간 뒤 남은 사람들이 나눈 대화까지 계속 읽혔다.** 컷오프를 경로마다 두는 대신 방 자체를 감춘다 — "어디까지 보여줄지"를 되풀이해 정하지 않아도 되고 목록과 결과가 어긋나지 않는다(이슈 #196). `chat_room_member` 행은 그대로 남는다(읽음 커서·`matchCount`·과거 SYSTEM 메시지 해석 근거).
...
- **이탈자의 읽음은 발행하지 않는다.** 이탈자는 `unreadCount`에 세지 않으므로 이벤트가 나가면 수신 측이 하나 더 뺀다. 커서는 기록한다 — 이탈자는 이제 방을 조회할 수 없어 이 경로로 들어올 일이 없지만, 나가기 직전에 들어온 요청이 커서를 되돌리지 않게 기록 자체는 남긴다.

...
- **웹 리다이렉트 애플 로그인**: 심사 요건은 앱에만 적용되고, 애플 웹 흐름은 `response_mode=form_post`(POST 콜백)라 지금의 GET 콜백 구조와 맞지 않는다. 웹은 카카오를 유지한다. → **이후 [ADR 0023](0023-apple-web-login-form-post-callback.md)에서 폼 POST 콜백을 추가해 뒤집었다.**

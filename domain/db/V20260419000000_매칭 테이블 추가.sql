...
CREATE TABLE group_match_room_member
(
    ...
    created_at DATETIME(6) NOT NULL COMMENT '생성일시',
    updated_at DATETIME(6) NOT NULL COMMENT '수정일시',
    CONSTRAINT pk_group_match_room_member PRIMARY KEY (id),
    CONSTRAINT group_match_room_member_uk_1 UNIQUE (room_id, member_id)
);

CREATE INDEX group_match_room_member_index_1 ON group_match_room_member (room_id);
CREATE INDEX group_match_room_member_index_2 ON group_match_room_member (member_id);

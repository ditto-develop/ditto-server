-- 채팅 기반 테이블 3종.
-- chat_room: 매칭(원본)당 채팅방 1개. source_type + source_id 로 원본을 가리키는 다형 FK (1:1=personal_match, 그룹=group_match).
-- chat_room_member: 방 참여자 + 회원별 마지막 읽은 메시지 위치(last_read_message_id).
-- chat_message: 방 메시지. id(단조 증가)로 정렬·커서 페이징한다.
CREATE TABLE chat_room
(
    id          BIGINT      NOT NULL AUTO_INCREMENT,
    source_type VARCHAR(20) NOT NULL COMMENT '원본 유형 (PERSONAL, GROUP)',
    source_id   BIGINT      NOT NULL COMMENT '원본 매칭 ID (personal_match 또는 group_match 의 ID)',
    created_at  DATETIME(6) NOT NULL,
    updated_at  DATETIME(6) NOT NULL,
    PRIMARY KEY (id),
    CONSTRAINT chat_room_uk_1 UNIQUE (source_type, source_id)
);

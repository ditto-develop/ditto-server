...
class AdminLoginServiceTest : FreeSpec({
    ...
    val oAuthClientFactory = mockk<OAuthClientFactory> {
        every { getClient(SocialProvider.KAKAO) } returns client
    }
    val memberSocialAccountService = mockk<MemberSocialAccountService>()
    val service = AdminLoginService(oAuthClientFactory, memberSocialAccountService)

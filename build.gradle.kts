...
tasks.register("verifyConventions") {
    ...
    doLast {
        // @SpringBootTest 직접 사용이 허용되는 베이스/지원 클래스
        val springBootTestWhitelist = setOf("IntegrationTest.kt", "RestDocsTest.kt", "AdminWebTest.kt")
        ...
        rootDir.walkTopDown()
            ...
            .forEach { file ->
                ...
                if (Regex("@(MockBean|SpyBean)").containsMatchIn(text)) {
                    violations += "$rel — @MockBean/@SpyBean 금지(MockK 사용). docs/testing/integration.md"
                }
                if (Regex("fun\\s+`[^`]*[가-힣][^`]*`").containsMatchIn(text)) {
                    violations += "$rel — 백틱 한글 메서드명 금지(@DisplayName 사용). docs/testing/integration.md"
                }
                if (file.name !in springBootTestWhitelist && "@SpringBootTest" in text) {
        ...
        file("domain/db").listFiles { _, name -> name.endsWith(".sql") }?.forEach { sql ->
            if (!Regex("^V\\d{14}_").containsMatchIn(sql.name)) {

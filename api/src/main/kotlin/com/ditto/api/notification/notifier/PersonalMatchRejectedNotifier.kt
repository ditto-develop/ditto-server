...
/**
 * 1:1 대화 신청이 거절되면 **신청한 사람에게만** 알린다 — 화면을 열고 있지 않으면 거절 사실을 알 길이
 * 없기 때문이다(조회 응답에는 `status: REJECTED` 가 이미 실려 있어 화면이 켜져 있으면 보인다).
 *
 * 거절한 사람에게는 보내지 않는다. 자기가 누른 것이라 알릴 것이 없다 — [ChatVoteClosedNotifier] 가
 * 마감자를 빼는 것과 같은 기준이다.
 *
 * 매칭 건마다 한 번이다(`MATCH_REJECTED` 의 `target_id` = `personal_match.id`). 거절 자체가
 * `PENDING` 에서만 일어나 두 번 날 수 없지만, 중복 정책을 유형에 박아 두면 적재 지점이 늘어도
 * 판단이 갈리지 않는다.
 *
 * **실패를 삼킨다.** 적재는 [NotificationAppender] 가 흡수하지만 그 앞의 닉네임 조회는 흡수 범위
 * 밖이라 여기서 막는다 — 거절 요청 경로에 있어 예외가 올라가면 이미 커밋된 거절이 실패로 보이고,
...
class PersonalMatchRejectedNotifier(
    ...
    private fun appendToRequester(matchId: Long, requesterId: Long, rejectedBy: Long): Boolean {
        // 닉네임이 없으면(탈퇴 등) 알리지 않는다 — "님과는 연결되지 않았어요"처럼 주어가 빈 문구가 나간다.
        val rejecterNickname = memberRepository.findById(rejectedBy).orElse(null)?.nickname ?: return false

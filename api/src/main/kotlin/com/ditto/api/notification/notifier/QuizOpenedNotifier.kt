...
/**
 * 이번 주 퀴즈가 열렸다고 활성 회원 전원에게 알린다. WeeklyNotificationScheduler 가 월요일에 부른다.
 *
...
class QuizOpenedNotifier(
    ...
    fun notifyOpened(now: LocalDateTime): Int =
        runCatchingExceptions { appendToActiveMembers(now) }
            .onFailure { logger.warn(it) { "퀴즈 오픈 알림 실패 — 무시한다: now=$now" } }
            .getOrDefault(0)

    private fun appendToActiveMembers(now: LocalDateTime): Int {
        val quizSet = quizSetRepository.findCurrentWeekActive(now).minByOrNull { it.id } ?: return 0

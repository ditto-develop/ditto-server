...
class ChatController(
    ...
    /** 읽음 처리. 커서가 전진했을 때만 READ 이벤트를 발행한다(트랜잭션 커밋 뒤). */
    ...
    /**
     * 1:1 채팅 종료 — 일반 매칭과 재매칭 방 모두 해당한다(그룹은 이 경로로 끝낼 수 없다).
     * 이미 끝난 방에 다시 요청해도 성공으로 답한다(더블 탭·재시도 대비).
     *
     ...
     * 재매칭 방은 평가를 열지 않으므로 이 호출이 곧바로 아무 일도 하지 않는다.
     */
    @Loggable
